...
```{r echo=FALSE, fig.width=4, fig.height=3, fig.cap="Sales Histogram"}
sales_hist <- readPNG("../images/histogram-sales.png")
grid.raster(sales_hist)
```

```{r echo=FALSE, fig.width=4, fig.height=3, fig.cap="TV Histogram"}
tv_hist <- readPNG("../images/histogram-tv.png")
grid.raster(tv_hist)
```

```{r echo=FALSE, fig.width=4, fig.height=3, fig.cap="Radio Histogram"}
...
```
From the estimate prediction above, we can see that for a $1,000 increase in TV advertising, there will be an increase in sales by around 203 units.
...
```
From the estimate prediction above, we can see that for a $1,000 increase in TV advertising, there will be an increase in sales by around 55 units.
  
When doing three seperate simple linear regression, this ignores the other two media in forming estimates for the regression coefficients, which leades to misleading estimates of the individual media effects on sales.
...
We can see that the newspaper regression coefficient estimate is non-zero from the simple regression (table above), but close to zero in the multiple regression model. Additionaly, notice that the p-value is no longer significant, with a value around 0.86.
...
_*1. Is at least one of the predictors useful in predicting the response?*_  
...
_* 2. Do all predictors help to explain the response, or is only a subset of the predictors useful? *_

Goal: finding the fitted linear model containing variables with low p-value

From the question above, computing the F-statistic and examining the associated p-value, we can determine if there are at least one predictor related to the response. Now, we want to know which of these predictors are useful. One way of approaching this question is to take a look at the individual p-values (TABLE). However, if p is large, then we are likely to make some false discoveries. It is possible to have all of the predictors associated with the response, however, most of the time, the response is only related to a subset of the predictors. Another way of selecting the "best" subset of predictors is by variable selection. We perform variable selection by trying out different models with different subset of predictors. In our case, we have 3 predictor variables $X_1$, $X_2$, and $X_3$.
...
The fitted linear model maximizes the correlation among all possible linear models.

_* 3. How well does the model fit the data? *_
...
######R^2 Statistic
The $R^2 Statistic$ provides an alternative measure of good fit to the RSE. It is a measure of the linear relationship between $X$ and $Y$, and has the range [0,1].
...
_* 4. How accurate is the prediction? *_

$\hat{\beta_0},\hat{\beta_1},\hat{\beta_2} and \hat{\beta_3}$



```{r echo=FALSE, fig.width=4, fig.height=3, fig.cap="Scatterplot Matrix"}
scatter_matrix <- readPNG("../images/scatterplot-matrix.png")
grid.raster(scatter_matrix)
```

```{r echo=FALSE, fig.width=4, fig.height=3, fig.cap="TV Regression on Sales"}
tv_sales <- readPNG("../images/scatterplot-tv-sales.png")
grid.raster(tv_sales)
```

```{r echo=FALSE, fig.width=4, fig.height=3, fig.cap="Radio Regression on Sales"}
radio_sales <- readPNG("../images/scatterplot-radio-sales.png")
grid.raster(radio_sales)
```

```{r echo=FALSE, fig.width=4, fig.height=3, fig.cap="Newspaper Regression on Sales"}
newspaper_sales <- readPNG("../images/scatterplot-newspaper-sales.png")
grid.raster(newspaper_sales)
```
